 ...
 *  Copyright (C) 2004-2006 created by fann_freegoldbar (at) yahoo dot com
 *
 *  This wrapper is fann_free software; you can redistribute it and/or
 ...
 *  2004-2006 created by <fann_freegoldbar@yahoo.com>.
 *
        ...
        */
            ...
            unsigned int num_output, fann_type **output)
        {
            ...
            struct fann_train_data *data =
                (struct fann_train_data *)fann_malloc(sizeof(struct fann_train_data));
            data->input = (fann_type **)fann_calloc(num_data, sizeof(fann_type *));
            data->output = (fann_type **)fann_calloc(num_data, sizeof(fann_type *));

            data->num_data = num_data;
            data->num_input = num_input;
            data->num_output = num_output;

        	fann_type *data_input = (fann_type *)fann_calloc(num_input*num_data, sizeof(fann_type));
        	fann_type *data_output = (fann_type *)fann_calloc(num_output*num_data, sizeof(fann_type));
...
private:
        /* Set the training data to the struct fann_training_data pointer.
            The struct has to be allocated with fann_malloc to be compatible
            with fann_destroy. */
    ...
    public:
        ...
        */ 
        ...
        /* Method: run
	        ...
	        This function appears in FANN >= 1.0.0.
        */ 
        fann_type* run(const fann_type *input)
        {
            if (ann == NULL)
            {
                return NULL;
        ...
        */ 
        fann_type* run_sse(const fann_type *input)
        {
            if (ann == NULL)
            {
                return NULL;
            }
            return fann_run_sse(ann, input);
        }
#endif
        ...
        /* Method: run_sse
	        ...
	        This function appears in FANN >= 2.1.0beta.
        */ 
        fann_type* run_avx(const fann_type *input)
        {
            if (ann == NULL)
            {
                return NULL;
            }
            return fann_run_avx(ann, input);
        }
#endif
        ...
        */ 
        ...
        /* Method: train
           ...
           Parameters:
   	        ...
   	        This function appears in FANN >= 1.0.0.
         */ 
        void train(const fann_type *input, const fann_type *desired_output)
        {
            if (ann != NULL)
            {
        ...
        /* Method: train_sse
           ...
           Parameters:
   	        ...
   	        This function appears in FANN >= 1.0.0.
         */ 
        void train_sse(const fann_type *input, const fann_type *desired_output)
        {
            if (ann != NULL)
            {
                fann_train_sse(ann, input, desired_output);
            }
        }
#endif
        ...
        /* Method: train_avx
           ...
           Parameters:
   	        ...
   	        This function appears in FANN >= 1.0.0.
         */ 
        void train_avx(const fann_type *input, const fann_type *desired_output)
        {
            if (ann != NULL)
            {
                fann_train_avx(ann, input, desired_output);
            }
        }
#endif
		/* Method: train_epoch
        ...
        */ 
        ...
        /* Method: get_activation_function
           ...
           Get the activation function for layer number *layer*, 
        ...
        activation_function_enum get_activation_function(int layer)
        {
            ...
            if (ann != NULL)
            {
                activation_function = fann_get_activation_function(ann, layer);
            }
		...
		*/
		bool can_use_sse()
		{
			return ann == NULL ? 0 : fann_can_use_sse(ann);
		}
		...
		*/
		...
		{
			if(ann != NULL)
				fann_disable_sse(ann);
		}

		/*  Method: can_use_avx
			Sets to non-zero ann->can_use_sse when ANN can be processed with AVX256 SIMD instructions. 
			To use SSE FANN must use floats, network must be fully connected (shortcuts should be ok too),
			and number of neurons in input and hidden layers including bias neurons must be multiply of 4.
			Also pointers to processing data must be aligned to 16 bytes boundary.

			Returns:
				Nonzero if ANN can be processed with AVX256 SIMD instructions
		...
		*/
		bool can_use_avx()
		{
			return ann == NULL ? 0 : fann_can_use_avx(ann);
		}

		/*  Method: disable_avx
			Disables AVX256 using by resetting ann->can_use_avx flag
		...
		*/
		...
		{
			if(ann != NULL)
				fann_disable_avx(ann);
		}

		bool isAvxOk()
		{
			return ann == NULL ? false : ann->can_use_avx;
		}
		bool isSseOk()
		{
			return ann == NULL ? false : ann->can_use_sse;
		}
