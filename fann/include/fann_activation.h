...
/* Function: tanh_approx
	Computes tanh approximation with formula tanh(x) ~ sign(x) * (1 - 1 / (1 + |x| + x^2 + a * x^4))

	This function appears in FANN >= 2.1.0beta.
*/ 
/*
__inline fann_type __fastcall tanh_approx (fann_type x)
{
	//http://www.kvraudio.com/forum/viewtopic.php?t=262823&postdays=0&postorder=asc&start=45
	//tanh(x) ~ sign(x) * (1 - 1 / (1 + |x| + x^2 + a * x^4))
	//a = 1

	const fann_type xa = fabs (x); 
	const fann_type x2 = x * x; 
	const fann_type x4 = x2 * x2; 
	const fann_type poly = xa + x2 + x4; 
	const fann_type resa = poly / (1 + poly); 
	const fann_type result = (x < 0) ? -resa : resa; 
	return result; 
}
*/
...
#if defined FLOATFANN && defined FANN_USE_SSE
	#define fann_activation_switch_ps(activation_function, value, result) \
	switch(activation_function) \
	{ \
		case FANN_LINEAR: \
			result = value; \
			break; \
		case FANN_SIGMOID: \
			result = fann_sigmoid_real_ps(value); \
			break; \
		case FANN_SIGMOID_SYMMETRIC: \
			result = fann_sigmoid_symmetric_real_ps(value); \
			break; \
		default: \
			assert(0); \
	}
#elif defined DOUBLEFANN && FANN_USE_SSE >= 0x20
	#define fann_activation_switch_pd(activation_function, value, result) \
	switch(activation_function) \
	{ \
		case FANN_TANH_APPROX: \
			result = tanh_approx_pd(value); \
			break; \
		case FANN_LINEAR: \
			result = value; \
			break; \
...
#if defined FLOATFANN && defined FANN_USE_AVX
	#define fann256_activation_switch_ps(activation_function, value, result) \
	switch(activation_function) \
	{ \
		case FANN_LINEAR: \
			result = value; \
			break; \
		case FANN_SIGMOID: \
			result = fann256_sigmoid_real_ps(value); \
			break; \
		case FANN_SIGMOID_SYMMETRIC: \
			result = fann256_sigmoid_symmetric_real_ps(value); \
			break; \
		default: \
			assert(0); \
	}
