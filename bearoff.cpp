...
#if !defined g_return_val_if_fail
#define g_return_val_if_fail(cond, ret) if(!(cond)) return (ret);
#endif

static int setGammonProb(const BgBoard * anBoard, unsigned int bp0, unsigned int bp1, float* g0, float* g1)
{
  int i;
  ...
  for(i = 5; i >= 0; --i) {
    tot0 += anBoard->anBoard[0][i];
    tot1 += anBoard->anBoard[1][i];
  }
  ...
  *g1 = 0.0;
  ...
  if( tot0 == 15 ) {
    struct GammonProbs* gp = getBearoffGammonProbs(anBoard->anBoard[0]);
    double make[3];

    if (BearoffDist ( BgEval::Instance()->pbc1, bp1, NULL, NULL, NULL, prob, NULL ))
		return -1;
  ...
  if( tot1 == 15 ) {
    struct GammonProbs* gp = getBearoffGammonProbs(anBoard->anBoard[1]);
    double make[3];

    if (BearoffDist ( BgEval::Instance()->pbc1, bp0, NULL, NULL, NULL, prob, NULL ))
		return -1;
...
static unsigned int HeuristicBearoff( char anBoard[ 6 ], const unsigned int anRoll[ 2 ] )
{
    ...
    return PositionId::PositionBearoff( anBoard, HEURISTIC_P, HEURISTIC_C );
}

static void GenerateBearoff( unsigned char *p, unsigned int nId )
{
    unsigned int anRoll[ 2 ], aProb[ 32 ];
	char anBoard[ 6 ];
	...
	{
	    PositionId::PositionFromBearoff( anBoard, nId, HEURISTIC_P, HEURISTIC_C );
...
{
    unsigned char *pm = (unsigned char *)malloc( 40 + 54264 * 64 );
    unsigned char *p;
    unsigned int i;
...
static int
BearoffEvalTwoSided ( const bearoffcontext *pbc, const BgBoard * anBoard, BgReward& arOutput ) 
{
	unsigned int nUs = PositionId::PositionBearoff ( anBoard->anBoard[ 1 ], pbc->nPoints, pbc->nChequers );
	unsigned int nThem = PositionId::PositionBearoff ( anBoard->anBoard[ 0 ], pbc->nPoints, pbc->nChequers );
	unsigned int n = PositionId::Combination ( pbc->nPoints + pbc->nChequers, pbc->nPoints );
...
static int BearoffEvalOneSided ( const bearoffcontext *pbc, const BgBoard *anBoard, BgReward& arOutput )
{
	int i, j;
	float aarProb[ 2 ][ 32 ];
	...
	for ( i = 0; i < 2; ++i ) 
	{
		an[ i ] = PositionId::PositionBearoff ( anBoard->anBoard[ i ], pbc->nPoints, pbc->nChequers );
...
BearoffEvalHypergammon ( const bearoffcontext *pbc, 
                         const BgBoard * anBoard, BgReward& arOutput ) 
{
	unsigned int nUs = PositionId::PositionBearoff ( anBoard->anBoard[ 1 ], pbc->nPoints, pbc->nChequers );
	unsigned int nThem = PositionId::PositionBearoff ( anBoard->anBoard[ 0 ], pbc->nPoints, pbc->nChequers );
	unsigned int n = PositionId::Combination ( pbc->nPoints + pbc->nChequers, pbc->nPoints );
...
int BearoffEval(const bearoffcontext * pbc, const BgBoard * anBoard, BgReward& arOutput)
{
	...
	default:
		perror("Invalid type in BearoffEval");
		return 0;
	}
}
...
extern void BearoffStatus(const bearoffcontext *pbc, char *sz)
{
	...
	{
		if (pbc->p)
			sprintf(buf, ("In memory 2-sided exact %d-chequer Hypergammon database evaluator"), pbc->nChequers);
		else
			sprintf(buf, ("On disk 2-sided exact %d-chequer Hypergammon database evaluator"), pbc->nChequers);

	}
	else
	{
		if (pbc->p)
			sprintf(buf, ("In memory %d-sided bearoff database evaluator"), pbc->bt);
		else
			sprintf(buf, ("On disk %d-sided bearoff database evaluator"), pbc->bt);
	...
	sprintf(buf, ("up to %d chequers on %d points (%d positions) per player"), pbc->nChequers, pbc->nPoints, PositionId::Combination(pbc->nChequers + pbc->nPoints, pbc->nPoints));
	sz += sprintf(sz, "   - %s\n", buf);
	...
	case BEAROFF_ONESIDED:
		if (pbc->fND)
			sz += sprintf(sz, "   - %s\n", ("distributions are approximated with a normal distribution"));
		if (pbc->fHeuristic)
			sz += sprintf(sz, "   - %s\n", ("with heuristic moves"));

		sz += sprintf(sz, "   - %s\n", pbc->fGammon
			? ("database includes gammon distributions")
			: ("database does not include gammon distributions"));
		break;
	case BEAROFF_HYPERGAMMON:
	case BEAROFF_INVALID:
	default:
	...
	sprintf(buf, ("number of reads: %lu"), pbc->nReads);
	sprintf(sz, "   - %s\n", buf);
}

static int BearoffDumpTwoSided ( const bearoffcontext *pbc, const BgBoard * anBoard, char *sz )
{
  unsigned int nUs = PositionId::PositionBearoff ( anBoard->anBoard[ 1 ], pbc->nPoints, pbc->nChequers );
  unsigned int nThem = PositionId::PositionBearoff ( anBoard->anBoard[ 0 ], pbc->nPoints, pbc->nChequers );
  unsigned int n = PositionId::Combination ( pbc->nPoints + pbc->nChequers, pbc->nPoints );
  ...
  sprintf ( sz + strlen(sz), "%19s %14s\n%s %12d  %12d\n\n",
	  ("Player"), ("Opponent"), ("Position"), nUs, nThem);
...
static int BearoffDumpOneSided ( const bearoffcontext *pbc, const BgBoard *anBoard, char *sz )
{
  unsigned int nUs = PositionId::PositionBearoff ( anBoard->anBoard[ 1 ], pbc->nPoints, pbc->nChequers );
  unsigned int nThem = PositionId::PositionBearoff ( anBoard->anBoard[ 0 ], pbc->nPoints, pbc->nChequers );
  float ar[ 2 ][ 4 ];
  ...
  sz += sprintf(sz, "%s\t%7.3f\t%7.3f\t\t", ("Mean"), ar[ 0 ][ 0 ], ar[ 1 ][ 0 ]);
  ...
  sz += sprintf(sz, "%s\t%7.3f\t%7.3f\t\t", ("Std dev"),
            ar[ 0 ][ 1 ], ar[ 1 ][ 1 ] );
  ...
  sz += sprintf(sz, "%s = %5.3f * %s\n%s = %s - %s\n\n",
	  ("EPC"), x, ("Average rolls"), ("Wastage"), ("EPC"), ("pips"));

  return 0;

}

static int BearoffDumpHyper( const bearoffcontext *pbc, const BgBoard * anBoard, char *sz )
{
  unsigned int nUs = PositionId::PositionBearoff ( anBoard->anBoard[ 1 ], pbc->nPoints, pbc->nChequers );
  unsigned int nThem = PositionId::PositionBearoff ( anBoard->anBoard[ 0 ], pbc->nPoints, pbc->nChequers );
  unsigned int n = PositionId::Combination ( pbc->nPoints + pbc->nChequers, pbc->nPoints );
  ...
  const char *aszEquity[] = {
   ("Owned cube"),
   ("Centered cube"),
   ("Centered cube (Jacoby rule)"),
   ("Opponent owns cube")
  };

  BgReward r;
  if ( BearoffHyper ( pbc, iPos, r, ar ) )
    return -1;
...
extern int BearoffDump(const bearoffcontext * pbc, const BgBoard * anBoard, char *sz)
{
	if(!pbc) return -1;
	...
	case BEAROFF_INVALID:
	default: 
		perror("Invalid type in BearoffDump");
		return -1;
	}
}
...
{
	pbc->map.open(pbc->szFilename);
	if (!pbc->map.is_open())
	{
		fprintf(stderr, "%s: Failed to map bearoffdatabase\n", pbc->szFilename);
		return NULL;
	}
	pbc->p = (unsigned char *)pbc->map.data();
	return pbc->p;
}
...
static void InvalidDb(bearoffcontext *pbc)
{
	if (errno)
	{
		...
		fprintf(stderr, "%s(%s): %s\n", ("Bearoff Database"), fn, strerror(errno));
	}
	BearoffClose(pbc);
}
/*
...
extern bearoffcontext *BearoffInit(const char *szFilename, const int bo, void (*p) (unsigned int))
{
	...
	if (!szFilename || !*szFilename) {
		fprintf(stderr, "%s\n", ("No database filename provided"));
		InvalidDb(pbc);
		return NULL;
	}
	pbc->szFilename = _strdup(szFilename);
	...
	if ((pbc->pf = fopen(szFilename, "rb")) == 0) {
		fprintf(stderr, "%s\n", ("Invalid or nonexistent database"));
		InvalidDb(pbc);
		return NULL;
	}
	/* 
	...
	if (!strncmp(sz + 6, "TS", 2))
		...
		fprintf(stderr, "%s: %s\n (%s: '%2s')\n", szFilename, ("incomplete bearoff database"), ("illegal bearoff type"), sz + 6);
		InvalidDb(pbc);
		return NULL;
	}
	...
	if (pbc->bt == BEAROFF_TWOSIDED || pbc->bt == BEAROFF_ONESIDED) 
	{
		...
		{
			fprintf(stderr, "%s: %s\n (%s: %d)\n", szFilename, ("incomplete bearoff database"),
				("illegal number of points"), pbc->nPoints);
			InvalidDb(pbc);
			return NULL;
		}
		...
		{
			fprintf(stderr, "%s: %s\n (%s: %d)", szFilename, ("incomplete bearoff database"),
				("illegal number of chequers"), pbc->nChequers);
			InvalidDb(pbc);
			return NULL;
		}
...
static unsigned short int *GetDistCompressed ( unsigned short int aus[ 64 ], const bearoffcontext *pbc, const unsigned int nPosID )
{
  ...
  unsigned int nPos = PositionId::Combination ( pbc->nPoints + pbc->nChequers, pbc->nPoints );
...
{
	if(!pbc) return -1;
	if(!(pbc->bt == BEAROFF_ONESIDED)) return -1;
	if (pbc->fND)
...
bool isBearoff(const bearoffcontext *pbc, const BgBoard * anBoard)
{
  ...
  if (!anBoard->anBoard[0][nOppBack] || !anBoard->anBoard[1][nBack])
    /* the game is over */
    return false;
  ...
  for ( i = 0; i <= nOppBack; ++i )
    nOpp += anBoard->anBoard[ 0 ][ i ];

  for ( i = 0; i <= nBack; ++i )
    n += anBoard->anBoard[ 1 ][ i ];
